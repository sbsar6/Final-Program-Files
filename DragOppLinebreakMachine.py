...
def machLearn(path):
    ...
    List = ["Unnamed:_0","LineBreakPrevPhase", "PhaseName", "Linebreak", "Tries","PhaseID"]
    columns = df.columns.values
    Features = [i for i in columns if str(i) not in List]
    bucket = df["Linebreak"].tolist()
    freqs = Counter(bucket)
    print(freqs)
    X =  np.array(df[Features].values)
    X = preprocessing.scale(X,axis=0)
        
    y = np.array(df["Linebreak"].values)
    ...
    bar = Bar(ranks,features,title="Dragons Feature Rankings Opp.Phase Linebreak",palette=brewer["Greens"][4], stacked=True)
    
    output_file("OppLinerankings.html")
    show(bar)
